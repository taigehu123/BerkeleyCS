    ...
    switch(instruction.itype.funct3) { // What do we switch on?
        ...
        case 0x5:
            switch((instruction.itype.imm & 0xFE0) >> 5) {
                case 0x20:
                    print_itype_except_load("srai", instruction, instruction.itype.imm & 0x01F);
                    break;
                case 0x00:
                    print_itype_except_load("srli", instruction, instruction.itype.imm & 0x01F);
                    break;
                default:
                    handle_invalid_instruction(instruction);
                    break;
...
void write_jal(Instruction instruction) {
    int final = bitSigner(get_jump_offset(instruction), 20);
    printf(JAL_FORMAT, instruction.ujtype.rd, final);
}
...
void print_itype_except_load(char *name, Instruction instruction, int imm) {
    int final = bitSigner(imm, 12);
    printf(ITYPE_FORMAT, name, instruction.itype.rd, instruction.itype.rs1, final);
}
...
void print_store(char *name, Instruction instruction) {
    printf(MEM_FORMAT, name, instruction.stype.rs2, get_store_offset(instruction), instruction.itype.rs1);
}

void print_branch(char *name, Instruction instruction) {
    int final = bitSigner(get_branch_offset(instruction), 12);
    printf(BRANCH_FORMAT, name, instruction.stype.rs1, instruction.stype.rs2, final);
}
