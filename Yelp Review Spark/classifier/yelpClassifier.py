...
class YelpClassifier(object):
    ...
    def review_to_word_counts(review):  
        review_id, star_num, review_text = review
        words = review_text.split(" ")
        word_counts = []
        for word in words:
            to_add = ((star_num, word), 1)
            word_counts.append(to_add)
        return word_counts
    ...
    def counts_to_probabilities(self, num_stars_and_word_counts):
        star_num = num_stars_and_word_counts[0][0]
        dict_to_add = {num_stars_and_word_counts[0][1]: self.compute_likelihood(num_stars_and_word_counts[1], int(star_num))}
        return (int(star_num), dict_to_add)
    ...
    def find_max_posterior(num_stars_posterior1, num_stars_posterior2):
        ...
        if num_stars_posterior1[1] > num_stars_posterior2[1]:
            return (num_stars_posterior1[0], num_stars_posterior1[1])
        else:
            return (num_stars_posterior2[0], num_stars_posterior2[1])
